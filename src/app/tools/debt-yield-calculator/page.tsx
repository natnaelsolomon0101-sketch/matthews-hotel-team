import * as React from "react";
import type { Metadata } from "next";
import AnswerPageView, { answerMetadata } from "@/components/answers/AnswerPageView";
import { page as tool } from "@/lib/data/tools/debt-yield-calculator";
import { page as paired } from "@/lib/data/answers/hotel-financing/loan-requirements";
import ToolIsland from "../ToolIsland";
import ToolExtras from "../ToolExtras";

export const metadata: Metadata = answerMetadata(tool, "/tools/debt-yield-calculator");

/**
 * Template D (geo/05-templates.md). Everything except <ToolIsland> is
 * server-rendered: the answer, the formula, the table, the worked example,
 * the FAQ and the sources are in the HTML without JavaScript.
 */
export default function Page() {
  return (
    <AnswerPageView
      page={tool}
      island={
        <>
          <ToolIsland kind="debt-yield-calculator" tool={tool.tool} />
          <ToolExtras
            page={tool}
            paired={{ href: "/hotel-financing/loan-requirements", label: paired.h1 }}
          />
        </>
      }
    />
  );
}
